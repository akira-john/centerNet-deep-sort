...
class Detector(object):
    ...
    def detect(self):
        ...
        bt = CentroidTracker(maxDisappeared=25, maxDistance=150)
        ct = CentroidTracker(maxDisappeared=25, maxDistance=250)
        start_00 = time.time()
        inq2 = mp.SimpleQueue()
        ...
        for _ in range(int(self.frames)):
            ...
            for c, (bbox_xywh, cls_conf) in enumerate(bbox_lis):
                if  c == 0:
                    ...
                    t_objects = bt.update(rects) if c==1 else ct.update(rects)
